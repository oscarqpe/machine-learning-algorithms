...
if opt.network == '' then
   ...
   if opt.model == 'convnet' then
      ...
      Penultima = nn.Linear(64*3*3, 200) 
      model:add(Penultima)
      model:add(nn.Tanh())
      Ultima = nn.Linear(200, #classes)
      model:add(Ultima)
      ------------------------------------------------------------
...
numFile = 0
function imprimirSalidas(dataset)
      -- local vars
   local time = sys.clock()
   ...
   for t = 1,dataset:size(),opt.batchSize do
      ...
      -- print(preds)
      -- print(targets)
      convertToCsv(Penultima,targets,numFile)
      -- confusion:
      for i = 1,opt.batchSize do
         confusion:add(preds[i], targets[i])
      end
...
function convertToCsv(capa , targets, name )
   local file = io.open( tostring(name) .. ".csv", "a")
...
-- filename = paths.concat(opt.save,'mnist.net')
...
print("------------------------------")
print("Imprimir Salidas")
print("------------------------------")
imprimirSalidas(trainData)
imprimirSalidas(testData)
