...
public:
    ...
    void train()
    {
        ...
        {
            _lrate_cnn = _lrate_cnn * 1;
            _lrate_mlp = _lrate_mlp * 1;
            
            cout<<"Epoca ["<<epo<<"] "<<_lrate_cnn<<"|"<<_lrate_mlp<<endl;
...
//                        cout<<endl;
//                        print_filter_convol(index + 1, 2, 1);
//                        print_filter_convol(index + 1, 2, 2);
//                        print_filter_convol(index + 1, 2, 3);
                        layer[index].process_convol2();
//                        print_image(index+1,1);
//                        print_image(index+1,2);
//                        print_image(index+1,3);
//                        print_image(index+1,4);
//                        print_image(index+1,5);
//                        print_image(index+1,6);
//                        print_image(index+1,7);
//                        print_image(index+1,8);
...
//  GENERAMOS LOS FILTROS DE LA CAPA SELECCIONADA
    void set_filter_by_layer(int index_layer)
    {
        int i = index_layer;
        int total_filter;
        int deep_filter;
        ...
        if (layer[i].type == tLayer::convol)
        {
            deep_filter =  layer[i].deep_filter;
    ...
    void set_filter_by_filter(int index_layer)
    {
        int i = index_layer;
        int total_filter;
        int deep_filter;
        ...
        {
            deep_filter =  layer[i].deep_filter;
            total_filter = layer[i].dim_filter * layer[i].dim_filter;
        }
        else{
            deep_filter =  1;
            total_filter = (layer[i].dim_input + 1) * layer[i].dim_out;
        ...
        for (int j = 0; j < layer[i].num_filter; j++)
        {
            for (int k = 0 ; k < deep_filter; k++)
            {
                int aux = layer[i].pos_filter_start + total_filter * deep_filter * j + k * total_filter;
                rand_distribucion_normal(aux, total_filter);
            }
        }
    }
